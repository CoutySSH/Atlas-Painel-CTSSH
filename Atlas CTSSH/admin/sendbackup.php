...
function backup_tables($host,$user,$pass,$name){
    ...
    // Save file
    $backupPath = sys_get_temp_dir() . '/atlasbackup.sql';
...
$backupPathTemp = sys_get_temp_dir() . '/atlasbackup.sql';
$backupPathHash = sys_get_temp_dir() . '/atlasbackup_' . $hash . '.sql';
if (file_exists($backupPathTemp)) {
    rename($backupPathTemp, $backupPathHash);
}
...
$path = $dominio."/admin/getbackup.php?key=".$hash;
...
if (file_exists($backupPathHash)) {
    unlink($backupPathHash);
}
?>
