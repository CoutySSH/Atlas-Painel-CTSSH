...
$user = $_REQUEST["user"];
list($username) = explode("?", explode("/", $user)[2]);
list($deviceId) = explode("=", explode("/", $user)[2]);
$mysqli = new mysqli($dbhost, $dbuser, $dbpass, $dbname);
$username = $mysqli->real_escape_string($username);
$query = $mysqli->query("SELECT * FROM ssh_accounts WHERE login ='" . $username . "'");
if ($query->num_rows < 0) {
    echo json_decode(["error" => "User not found"]);
    exit;
}
...
foreach ($deviceIdData as $deviceData) {
    if ($deviceData["deviceid"] == $deviceId) {
        $foundDeviceId = true;
        if (!$foundDeviceId && $limitConnections < count($deviceIdData)) {
            $limitReached = true;
        }
        if (!$limitReached && !$foundDeviceId) {
            ...
            if (!$query) {
                echo json_decode(["error" => "Failed to insert device ID"]);
                exit;
            }
            $deviceIdData[] = ["nome_user" => $username, "deviceid" => $deviceId];
        }
        $query = $mysqli->query("SELECT * FROM ssh_accounts WHERE login ='" . $username . "'");
        if (0 < $query->num_rows) {
